...
export default class Ping {
  ...
  startRequestLoop() {
    const doIteration = async (count = 0, allTime = 0) => {
      ...
      console.log(`${agent.options.proxy.host} Время выполнения запроса: ${info.timeMs}, bytes: ${info.bytes}, среднее время: ${Math.round(allTime / count)}, медиана: ${getMedian(this.requestTimes)}`);
