...
export default class Proxy6 {
  ...
  async getProxies(ipVersion = '4', country, ip) {
    const filters = [
      (proxy) => proxy.version === ipVersion,
      (proxy) => {
        if (country) return proxy.country === country;
        return true;
      },
      (proxy) => {
        if (ip) return proxy.host === ip;
        return true;
      },
    ];
    ...
    const proxies = Object.values(response.body.list)
      .filter((proxy) => filters.every((filter) => filter(proxy)));
    // if (country) {
  ...
  async getAgents({ ipVersion, country, ip }) {
    const proxies = await this.getProxies(ipVersion, country, ip);
